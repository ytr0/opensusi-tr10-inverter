...
右下が温度センサ。緑の枠が N ウェル、その中に DP ダイオードが9個並ぶ。中央の1個が TEMP1、
残り8個を上のバスでまとめて TEMP8。斜線の帯は M2 の遮光板。左から回り込む配線が VSS レール。
...
- 測り方: TEMP1 と TEMP8 に同じ電流（10µA 程度）を流し、電圧差 ΔVbe を読む。25℃ で約 58mV、感度 +0.193mV/℃。
  `ngspice postlayout/tb_tempsens.spice` で、抽出ネットリストの温度特性を表示できる（-20〜100℃、直線）。
